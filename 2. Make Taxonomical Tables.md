QMP Absolute quantification project: Make Taxonomical Tables
=====================================
...
```
library (vegan)
IBD_sample_subset <- read.table("IBD_sample_subset_final.txt", header = T, sep="\t", as.is=TRUE, row.names = 1, check.names = F)
```
...
taxas = colnames(IBD_sample_subset)
mini = head(taxas)
strsplit(mini, "\\|")
```
...
split_taxonomy_level(IBD_sample_subset, 1)
